...
test('contestServices.changeOrder() properly changes order of an array', () => {
    ...
    expect(changeOrder(["a", {name: "b"}, "c", {description: "d"}, "e"], [3, 4, 1, 2])).toEqual(["a", {description: "d"}, "e", {name: "b"}, "c"])
})

//=================| ERROR THROWING |===============
test("contestServices.changeOrder() properly throws 'Exceeded' error", () => {
    // when order input is above it's proper range
    expect(() => changeOrder([0, {count: 1}, 2, 3, 4, 5, 6], [4, `2`, 5, "1"])).toThrow("Exceeded")
    expect(() => changeOrder([0, 1, {count: 2}, {count: 3}, 4], ['4', 2, 5, "1", 4])).toThrow("Exceeded")
    // when order input is below it's proper range
    expect(() => changeOrder([0, 1, 2, 3, {count: 4}, 5], ['4', `-1`, 3, "1"])).toThrow("Exceeded")
})
...
test("contestServices.changeOrder() properly throws 'NaI' error", () => {
    ...
    expect(() => changeOrder([0, 1, 2, {count: 3}, 4, 5], ['4', `1`, 3.2, "1"])).toThrow("NaI")
})

test("contestServices.changeOrder() properly throws 'Duplicate' error", () => {
    // when order input is duplicated
    expect(() => changeOrder([0, 1, 2, 3, 4, 5, {count: 6}], ['4', `1`, 3, "3", 2])).toThrow("Duplicate")
})